...
func (h *IPTablesHandler) CreateAndEnsureIPTablesChains() error {
	var err error
	ipt := h.ipt
	...
	vxlanUdpRuleSpec := []string{"-p", "udp", "-m", "udp", "--dport", "5189", "-j", "ACCEPT"}
...
func getPostroutingRules(isGateway bool, tep *netv1alpha1.TunnelEndpoint) ([]string, error) {
	...
	if isGateway {
		...
		return []string{
			strings.Join([]string{"!", "-s", localPodCIDR, "-d", remotePodCIDR, "-j", "SNAT", "--to-source", natIP.String()}, " "),
			//strings.Join([]string{"-s", localPodCIDR, "-d", remotePodCIDR, "-j", "ACCEPT"}, " "),
		}, nil
	}
	return []string{

...
const (
	...
	secretName        = "wireguard-pubkey" // name of the secret that contains the public key used by wireguard
	SecretLabel       = "net.liqo.io/key"  // label for the secret that contains the public key
	defaultPort       = 5871
...
func (w *wireguard) setKeys(c *k8s.Clientset, namespace string) error {
	...
	if apierrors.IsNotFound(err) {
		...
		pKey := corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:      secretName,
				Namespace: namespace,
				Labels:    map[string]string{SecretLabel: DriverName},
			},
		...
		if err != nil {
			return fmt.Errorf("failed to create the secret with name %s: %v", secretName, err)
		}
		return nil
	}
	...
	if !found {
		return fmt.Errorf("no data with key '%s' found in secret %s", PrivateKey, secretName)
	}
	...
	if !found {
		return fmt.Errorf("no data with key '%s' found in secret %s", PublicKey, secretName)
	}

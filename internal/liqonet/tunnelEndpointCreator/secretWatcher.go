...
func setSecretFilteringLabel(options *metav1.ListOptions) {
	...
	if options.LabelSelector == "" {
		newLabelSelector := []string{wireguard.SecretLabel, "=", wireguard.DriverName}
		options.LabelSelector = strings.Join(newLabelSelector, "")
	} else {
		newLabelSelector := []string{options.LabelSelector, wireguard.SecretLabel, "=", wireguard.DriverName}
		options.LabelSelector = strings.Join(newLabelSelector, "")
	}
}
